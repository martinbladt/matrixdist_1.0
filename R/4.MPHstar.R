...
MPHstar <- function(  alpha = NULL,
  ...
  if(all(is.null(R))){R <- random_reward(dim(S)[1], variables)
  Rname="random"
  }else{
    if (dim(R)[1] != dim(S)[1]) {
...
find_weight=function(x){
  ...
  for (i in unique) {
    ...
    if(i>0){cum_weight <- c(cum_weight, length(which(x == i)))}
  }
...
MPHstar_data_aggregation <- function(y,w=numeric(0), rc=numeric(0)) {
  ...
  if(is.matrix(y) & length(y)>1){
    ...
    if(length(rc)==0){rc=matrix(rep(1,nrow(y)*ncol(y)),nrow(y),ncol(y))} #no right censoring
    ...
    if(n1>1 & n2>1) {mat[[1]]=list(un=find_weight(sumData[un_obs]),rc=find_weight(sumData[rc_obs]))
    
    }else if(n1==1 & n2>1){ mat[[1]]=list(un=find_weight(sum(y[,un_obs])),rc=find_weight(sumData[rc_obs]))
    
    }else if(n1==0 & n2>1){ mat[[1]]=list(un=find_weight(NULL),rc=find_weight(sumData[rc_obs]))
    
    }else if(n1>1 & n2==1){ mat[[1]]=list(un=find_weight(sumData[un_obs]),rc=find_weight(sum(y[,rc_obs])))
    
    }else if(n1>1 & n2==0){ mat[[1]]=list(un=find_weight(sumData[un_obs]),rc=find_weight(NULL))
    
    }else if (n1==1 & n2==1){ mat[[1]]=list(un=find_weight(sum(y[,un_obs])),rc=find_weight(sum(y[,rc_obs])))}
  ...
  for(i in 1:ncol(y)){
    ...
    if(length(w)==0) {mat[[m]]=list(un=find_weight(m_y[which(m_rc==1)]),
                                    rc=find_weight(m_y[which(m_rc==0)]))}
    if(length(w)>0){
      m_w=w[,i]
      
      mat[[m]]=list(un=cbind(m_y[which(m_rc==1)],m_w[which(m_rc==1)]),
...
setMethod(
  ...
  function(x,
    ...
    if(!is.matrix(y))stop("Please use a matrix with marginal observations")
    if(!all(y> 0)) stop("data should be positive")
    if(!all(weight >= 0)) stop("weights should be non-negative")
    if(!all(x@pars$R>=0))stop("rewards should be non-negative")
    if(all(x@pars$R==0))stop("A reward matrix of zeros results in an atom at zero with probability 100%")
    ...
    if (is_mph) {
      for (k in 1:stepsEM) {
        ...
        if(r < 1){
          ...
          if(length(rc)>0){ r_rc <- rc[indices,]}
          if(length(weight)>0){ r_weight <- weight[indices,]}
          
          B <- MPHstar_data_aggregation(r_y,r_weight, r_rc)
          
        }
        
        epsilon <- if(!is.na(uni_epsilon)){uni_epsilon} else{1e-4}
