...
//' This is the fastest option
// [[Rcpp::export]]
double logLikelihoodMLogNormal_RKs(double h, NumericVector & pi, NumericMatrix & T, double beta, const NumericVector & obs, const NumericVector & weight, const NumericVector & rcens, const NumericVector & rcweight, const NumericVector & scale1, const NumericVector & scale2) {
  long p{T.nrow()};
  NumericMatrix m_pi(1,p, pi.begin());
...
//' This is the fastest option
// [[Rcpp::export]]
double logLikelihoodMLogNormal_RKs1(double h, NumericVector & pi, NumericMatrix & T, double beta, const NumericVector & obs, const NumericVector & weight, const NumericVector & rcens, const NumericVector & rcweight, const NumericVector & scale1, const NumericVector & scale2) {
  long p{T.nrow()};
  NumericMatrix m_pi(1,p, pi.begin());
  ...
  // Non censored data
  if (obs.size() > 0) {
    dt = pow(log(scale1[0] * obs[0] + 1), beta);
  }
  for (int k{0}; k < obs.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, t)(0,0);
    logLh += weight[k] * (log(density) + log(scale1[k]) + log(beta) + (beta -1) * log(log(scale1[k] * obs[k] + 1)) - log(scale1[k] * obs[k] + 1));
    if (k < obs.size() - 1){dt = pow(log(scale1[k + 1] * obs[k + 1] + 1), beta) - pow(log(scale1[k] * obs[k] + 1), beta);}
  }
  //Right censored data
  if (rcens.size() > 0) {
    dt = pow(log(scale2[0] * rcens[0] + 1), beta);
    avector = clone(m_pi);
  }
  for (int k{0}; k < rcens.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, e)(0,0);
    logLh += rcweight[k] * log(density);
    if (k < rcens.size() - 1){dt = pow(log(scale2[k + 1] * rcens[k + 1] + 1), beta) - pow(log(scale2[k] * rcens[k] + 1), beta);}
  }
...
//' Loglikelihood of matrix Pareto using RK
// [[Rcpp::export]]
double logLikelihoodMPar_RKs_double(double h, NumericVector & pi, NumericMatrix & T, const NumericVector & beta1, const NumericVector & beta2, const NumericVector & obs, const NumericVector & weight, const NumericVector & rcens, const NumericVector & rcweight, const NumericVector & scale1, const NumericVector & scale2) {
  long p{T.nrow()};
  NumericMatrix m_pi(1,p, pi.begin());
  ...
  // Non censored data
  if (obs.size() > 0) {
    dt = scale1[0] * log(obs[0] / beta1[0] + 1);
  }
  for (int k{0}; k < obs.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, t)(0,0);
    logLh += weight[k] * (log(density) + log(scale1[k]) - log(obs[k] + beta1[k]));
    if (k < obs.size() - 1){dt = scale1[k + 1] * log(obs[k + 1] / beta1[k + 1] + 1) - scale1[k] * log(obs[k] / beta1[k] + 1);}
  }
  //Right censored data
  if (rcens.size() > 0) {
    dt = scale2[0] * log(rcens[0] / beta2[0] + 1);
    avector = clone(m_pi);
  }
  for (int k{0}; k < rcens.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, e)(0,0);
    logLh += rcweight[k] * log(density);
    if (k < rcens.size() - 1){dt = scale2[k + 1] * log(rcens[k + 1] / beta2[k + 1] + 1) - scale2[k] * log(rcens[k] / beta2[k] + 1);}
  }
...
//' This is the fastest option
// [[Rcpp::export]]
double logLikelihoodMLogNormal_RKs_double(double h, NumericVector & pi, NumericMatrix & T, const NumericVector & beta1, const NumericVector & beta2, const NumericVector & obs, const NumericVector & weight, const NumericVector & rcens, const NumericVector & rcweight, const NumericVector & scale1, const NumericVector & scale2) {
  long p{T.nrow()};
  NumericMatrix m_pi(1,p, pi.begin());
  ...
  // Non censored data
  if (obs.size() > 0) {
    dt = scale1[0] * pow(log(obs[0] + 1), beta1[0]);
  }
  for (int k{0}; k < obs.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, t)(0,0);
  ...
  //Right censored data
  if (rcens.size() > 0) {
    dt = scale2[0] * pow(log(rcens[0] + 1), beta2[0]);
    avector = clone(m_pi);
  }
  for (int k{0}; k < rcens.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, e)(0,0);
    logLh += rcweight[k] * log(density);
    if (k < rcens.size() - 1){dt = scale2[k + 1] * pow(log(rcens[k + 1] + 1), beta2[k + 1]) - scale2[k] * pow(log(rcens[k] + 1), beta2[k]);}
  }
...
//' Loglikelihood of matrix Gompertz using RK
// [[Rcpp::export]]
double logLikelihoodMGomp_RKs_double(double h, NumericVector & pi, NumericMatrix & T, const NumericVector & beta1, const NumericVector & beta2, const NumericVector & obs, const NumericVector & weight, const NumericVector & rcens, const NumericVector & rcweight, const NumericVector & scale1, const NumericVector & scale2) {
  long p{T.nrow()};
  NumericMatrix m_pi(1,p, pi.begin());
  ...
  // Non censored data
  if (obs.size() > 0) {
    dt = scale1[0] * (exp(obs[0] * beta1[0]) - 1) / beta1[0];
  }
  for (int k{0}; k < obs.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, t)(0,0);
    logLh += weight[k] * (log(density) + log(scale1[k]) + obs[k] * beta1[k]);
    if (k < obs.size() - 1){dt = scale1[k + 1] * (exp(obs[k + 1] * beta1[k + 1]) - 1) / beta1[k + 1] - scale1[k] * (exp(obs[k] * beta1[k]) - 1) / beta1[k];}
  }
  //Right censored data
  if (rcens.size() > 0) {
    dt = scale2[0] * (exp(rcens[0] * beta2[0]) - 1) / beta2[0];
    avector = clone(m_pi);
  }
  for (int k{0}; k < rcens.size(); ++k) {
    if(dt > 0) a_rungekutta(avector, dt, h, T);
    density = matrix_product(avector, e)(0,0);
    logLh += rcweight[k] * log(density);
    if (k < rcens.size() - 1){dt = scale2[k + 1] * (exp(rcens[k + 1] * beta2[k + 1]) - 1) / beta2[k + 1] - scale2[k] * (exp(rcens[k] * beta2[k]) - 1) / beta2[k];}
  }
