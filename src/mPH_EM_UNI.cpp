...
void EM_step_mPH_rc (arma::vec & alpha, Rcpp::List & S_list, const arma::mat y, const arma::mat delta, double h){
  ...
  for(int i{0}; i<d; ++i){
    ...
    for(int j{0}; j<p; ++j){
      arma::rowvec e_k(p); e_k.zeros();
      e_k(j)=1;
      
      arma::vec obs=y.col(i);
      arma::cube m_exp(p, p, n); m_exp.zeros();
  ...
  for( int k{0}; k<p; ++k){
    for(int i{0}; i<d; ++i){
      ...
      for(int m{0}; m<n; ++m){
        ...
        //a_kij[m][k][i][]
        arma::vec step_vec(p); step_vec.zeros();
        for(int j{0}; j<p; ++j){
  ...
  for(int k{0}; k<p; ++k){
    for(int i{0}; i<d; ++i){
      for(int m{0}; m<n; ++m){
        arma::cube copy=a_ki; copy.shed_slice(i);
  ...
  for(int k{0}; k<p; ++k){
    for(int i{0}; i<d; ++i){
      ...
      arma::mat copy=step_mat; copy.shed_col(k);
